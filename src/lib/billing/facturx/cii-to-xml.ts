...
export function crossIndustryInvoiceToXml(invoice: CrossIndustryInvoice): string {
  ...
  const body = [
    ...
    `<rsm:SupplyChainTradeTransaction>${invoice.supplyChainTradeTransaction.includedSupplyChainTradeLineItem.map(renderLineItem).join("")}<ram:ApplicableHeaderTradeAgreement><ram:SellerTradeParty>${renderTradeParty(invoice.supplyChainTradeTransaction.applicableHeaderTradeAgreement.sellerTradeParty)}</ram:SellerTradeParty><ram:BuyerTradeParty>${renderTradeParty(invoice.supplyChainTradeTransaction.applicableHeaderTradeAgreement.buyerTradeParty)}</ram:BuyerTradeParty></ram:ApplicableHeaderTradeAgreement><ram:ApplicableHeaderTradeDelivery></ram:ApplicableHeaderTradeDelivery><ram:ApplicableHeaderTradeSettlement>${codeTag("ram:InvoiceCurrencyCode", settlement.invoiceCurrencyCode)}${settlement.applicableTradeTax.map(renderTradeTax).join("")}<ram:SpecifiedTradeSettlementHeaderMonetarySummation>${amountTag("ram:LineTotalAmount", summation.lineTotalAmount)}${amountTag("ram:TaxBasisTotalAmount", summation.taxBasisTotalAmount)}${summation.taxTotalAmount.map((tax) => amountTag("ram:TaxTotalAmount", tax)).join("")}${amountTag("ram:GrandTotalAmount", summation.grandTotalAmount)}${amountTag("ram:DuePayableAmount", summation.duePayableAmount)}</ram:SpecifiedTradeSettlementHeaderMonetarySummation></ram:ApplicableHeaderTradeSettlement></rsm:SupplyChainTradeTransaction>`,
  ].join("");
